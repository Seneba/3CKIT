observeEvent(input$Normalize,
             if(input$Normalize== 1){
               output$normalisation <- renderPlot({

                 ff <- read.flowSet(input$Files$datapath)
                 ff <- CATALYST::concatFCS(ff)
                 
                 #normalisation
                 normCytof(x=ff, y=input$beads, k=80, plot=TRUE)
...
output$Normalisation <- downloadHandler(
  # This function returns a string which tells the client
  # browser what name to use when saving the file.
  filename = function() {
    paste("Normalized Data",".Zip" ,sep = "")
  },
  ...
  content = function(file){
    ...
    ff <- concatFCS(ff)
    #normailisation
    ff <- normCytof(x=ff, y="dvs", k=80, plot=FALSE)
    
    #fnames <-  file.path(tempdir(), "compensate.fcs")
    tmp.name <- paste0("Flowset_normalized",".fcs")
    fnames <- c(unlist(fnames), tmp.name)
    write.flowSet(ff,fnames)
...
cellCheck <- reactive({
  if(is.null(ordFCS()))
    return(NULL)
  if(is.null(TimeChCheck())){
    flowRateData <- flow_rate_bin(ordFCS(), second_fraction = 0.1,
  ...
  flowSignalData <- flow_signal_bin(ordFCS(), channels = NULL, 
                                    binSize = optSize, timeCh = timeChannel(), 
